...
def data_to_canvas(c, width, height,id_order, email,name):
   ...
   c.drawString(inch, of_height, email)
   ...
   c.drawString(inch, of_height , name)
   ...
   of_height -= 15
   c.setFont("Helvetica-Bold", 9)
   c.drawString(width / 2.5, of_height, "")




def credit_card(canvas,num_cradit_card,name_card,total_price,name,Address,City,Zip,Mobile):
   textobject = canvas.beginText()
   textobject.setTextOrigin(inch, inch+17)
   textobject.setFont("Helvetica-Oblique", 10)
   textobject.setFillGray(0.4)
   ...
   textobject.setFillGray(0.4)
   """**** **** **** """
   textobject.textLines(f"{name_card}" + ""+num_cradit_card)
   textobject.setTextOrigin(width / 2.6, inch+17)
   textobject.setFont("Helvetica-Oblique", 10)
...
def create_pdf(items,_user,shipping):
   file_name = f"{items.get_order_id()}.pdf"
   with open(file_name, "w+") as create:
        create.write("")
   last_item = len(items.get_dresses_list())
   last_item += len(items.get_swimwear_list())
   last_item += len(items.get_general_list())
   ...
   if  len(items.get_dresses_list()) > 0 or len(items.get_swimwear_list()) > 0:
        ...
        print("w: ", width, "h: ", height)#updata to database and if name is empty than
        """shipping_to is object sent from js when user sent the order"""
        credit_card(my_canvas,shipping['credit_card'],shipping['name_card'],total_price,shipping['name'],
        ...
        for it in items.get_general_list():
             add_items(my_canvas, start_y, it)
             start_y += 1.5

        my_canvas.setPageSize((width, (save_height + 300)))
        data_to_canvas(my_canvas, width,save_height+100 , items.get_order_id(),_user.email,_user.name)
        my_canvas.showPage()
        my_canvas.save()
