...
from main import Swimwear, Dresses, Users, Ordering,Current_Order, Sdk_Product
import invoice_pdf
import send_emails
...
def get_database():
    clients_ = MongoClient("mongodb+srv://username:password@websw.dfksw.mongodb.net/?retryWrites=true&w=majority")
...
def open_home():
    ...
    if 'user_id' in session:
        if len(user.email) > 0:
            ...
            set_user_with_id()
    if len(temp_list_home_new_product) == 0:
        ...
        for it in cursor:
            ...
            temp_list_home_new_product.append(doc)
            list_home_mew_items.append(it)

    return render_template('Home.html', list_home_new_product_=temp_list_home_new_product)

get_sku = Sdk_Product()
single_ = []
@app.route('/home/new/items/display', methods=['GET','POST'])
def home_new_items():
    ...
    if request.method == 'POST':  # user add to cart
        ...
        get_sku.set_sku(sku)
        for it in list_home_mew_items:
            if get_sku.get_sku() == it['sku']:
                print(it['sku'])
                return jsonify({'this_item': 'ok'})
        return jsonify({'this_item': 'error'})
    if len(single_) == 0 and get_sku.get_sku() != None:
       for it in list_home_mew_items:
           if get_sku.get_sku() == it['sku']:
               single_.append(it)
               break
    return render_template('SingleItem.html', single_item=single_, may_like_=temp_list_home_new_product)
...
list_dresses = []
temp_list_dresses = []
@app.route('/home/dresses/db/download',methods=['GET','POST'])
def start_download_dresses():
    if len(list_dresses) == 0:
         db = clients['DesignerClothes']
    ...
    return render_template('Dresses.html',temp_list_dresses=temp_list_dresses)
...
_single_item = []
sku_ = Sdk_Product()
@app.route('/home/products/display/item', methods=['GET','POST'])
def set_ticket_dresses():
    if request.method == 'POST':  # user add to cart
        ...
        sku_.set_sku(sku[0]+sku[1])
        print("sku",type(sku), sku[0],sku[1])
        if sku[0] == 'D' and sku[1] == 'R':
           ...
           return jsonify({'this_item':'error'})
            ...
            if len(_single_item) > 0:
                return jsonify({'this_item':'ok'})
            return jsonify({'this_item':'error'})
    elif sku_.get_sku() == 'SW':
         print("swin")
         return render_template('SingleItem.html', single_item=_single_item, may_like_=temp_list_swimwear_to)
    elif sku_.get_sku() == 'DR':
         print("dress")
         return render_template('SingleItem.html', single_item=_single_item, may_like_=temp_list_dresses)
    else:
        return render_template('SingleItem.html', single_item='', may_like_=[])
...
def convert_to_pdf():
    ...
    if request.method == "POST":
        if "id_order" in session:
           ...
           pay = request.get_json()#return the object
